...
export default function AdminPanel() {
  ...
  const [backfillChatSenderUidStatus, setBackfillChatSenderUidStatus] =
    useState("");
  ...
  const [classesTeachersOverview, setClassesTeachersOverview] = useState([]);
  const [classesTeachersLoading, setClassesTeachersLoading] = useState(false);
  ...
  async function handleToggleGlobalDarkMode(on) {
    ...
    try {
      ...
      setGlobalDarkModeStatus(
        on
          ? "Dark mode is on for the entire site."
          : "Dark mode is off for the entire site.",
      );
  ...
  async function handleToggleGlobalPinkMode(on) {
    ...
    try {
      ...
      setGlobalPinkModeStatus(
        on
          ? "Pink mode (Valentine's) is on for the entire site."
          : "Pink mode is off.",
      );
  ...
  useEffect(() => {
    if (
      ...
      return;
    let cancelled = false;
    setClassesTeachersLoading(true);
    (async () => {
      try {
        ...
        const overview = await Promise.all(
          classesList.map(async (cls) => {
            const ref = doc(db, "classDescriptions", cls.id);
            const snap = await getDoc(ref);
            const teacherIds =
              (snap.exists() && snap.data().classInfo?.teacherIds) || [];
        ...
        if (!cancelled) setClassesTeachersOverview(overview);
      } catch (e) {
        if (!cancelled) setClassesTeachersOverview([]);
      } finally {
        if (!cancelled) setClassesTeachersLoading(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (selected) {
      ...
      setEditClass(
        selected.graduate ||
          (selected.class && /^graduated?$/i.test(String(selected.class)))
          ? "graduated"
          ...
          ? Array.from(
              new Set([
                ...selected.roles.map((r) => {
                  ...
                  return lower === "teacher" ? "professor" : lower;
                }),
                "user",
              ]),
            )
  ...
  async function handleSaveUserEdit() {
    ...
    try {
      const ref = doc(db, "users", selected.uid);
      const classVal = String(editClass || "")
        .trim()
        .toLowerCase();
    ...
    try {
      ...
      while (hasMore) {
        const q = lastDoc
          ...
          : query(messagesRef, orderBy("timestamp", "desc"), limit(READ_BATCH));
    ...
    >
      ...
      {roles.includes("admin") && (
        <div style={sectionBox}>
          {sectionTitle(
            "Site theme",
            "Turn dark or pink mode on or off for the entire site.",
          )}
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button
                ...
                style={{
                  background: globalPinkMode
                    ? "linear-gradient(135deg, #c75d7a 0%, #d48494 100%)"
                    : "linear-gradient(135deg, #e8a0b0 0%, #f0b0c0 100%)",
                  color: "#fff",
      ...
      {roles.includes("admin") && (
        <div style={sectionBox}>
          {sectionTitle(
            "Starshade Hall – background music",
            "Set a YouTube link that plays as background music when users enter Starshade Hall. Only the audio is played; the video is not shown. Users can stop or start the music on the page.",
          )}
          <label style={labelStyle}>
            YouTube link (e.g. watch or youtu.be)
          </label>
          <input
            type="url"
      ...
      {roles.includes("admin") && (
        <div style={sectionBox}>
          {sectionTitle(
            "Rules & shop",
            "Edit site rules and manage shop items or books.",
          )}
          <RulesAdmin />
        </div>
      )}
        ...
        <div style={sectionBox}>
          ...
          {showBanned && (
            ...
            >
              ...
              {ipBanStatus && (
                <div style={{ color: "#ffd86b" }}>{ipBanStatus}</div>
              )}
            </div>
          )}
          ...
          >
            {vipUsers.length === 0 && (
              <li style={{ color: theme.secondaryText }}>
                No users with active VIP.
              </li>
            )}
            {vipUsers.map((u) => {
              const expiresMs = getVipExpiresAtMs(u);
              const expiresStr =
                expiresMs != null ? new Date(expiresMs).toLocaleString() : "";
              return (
                <li
                  key={u.uid}
                  style={{
      ...
      <div style={sectionBox}>
        {sectionTitle(
          "Users",
          "Search for a user, then edit profile, roles, currency, or moderation.",
        )}
        <label htmlFor="admin-user-search" style={labelStyle}>
          ...
          {filtered.slice(0, 20).map(
            (
              ...
              >
                {u.displayName || u.email || u.uid}{" "}
                <span
                  style={{ color: theme.secondaryText, fontSize: "0.9rem" }}
                >
                  ({u.currency ?? 0} nits)
                </span>
              </li>
        ...
        {selected && roles.includes("admin") && (
          <div
            style={{ borderTop: `2px solid ${theme.border}`, paddingTop: 20 }}
          >
            <h3
              style={{
                color: theme.secondaryText,
            ...
            <div style={userCard}>
              ...
              <div>
                ...
                >
                  <span
                    style={{ fontSize: "0.9rem", color: theme.secondaryText }}
                  >
                    Remove one role:
                  </span>
                  <select
                  ...
                  >
                    ...
                    {REMOVABLE_ROLES.filter((r) => editRoles.includes(r)).map(
                      (r) => (
                        <option key={r} value={r}>
                          {r}
                        </option>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      removeSelectedRole(e);
                    }}
                    disabled={!roleToRemove}
                    style={{
                      padding: "6px 12px",
                      fontSize: "0.9rem",
                      background: roleToRemove
                        ? "rgba(139, 0, 0, 0.25)"
                        : theme.background,
                      color: theme.text,
                      border: "1px solid " + theme.border,
                ...
                >
                  The &quot;user&quot; role is always kept. Use the dropdown to
                ...
                <div>
                  ...
                  >
                    ...
                    {LEADABLE_ROLES.filter(({ value }) => {
                      const alreadyLead = users.find(
                        (u) =>
                          u.uid !== selected?.uid && u.leadForRole === value,
                      );
              ...
              {editUserStatus && (
                <div style={{ color: theme.secondaryText, fontSize: "0.9rem" }}>
                  {editUserStatus}
                </div>
              )}
            </div>
            ...
            <div style={userCard}>
              ...
              >
                Remove all active potion effects (dark mode, love, sparkle,
                etc.) from selected user.
              </p>
              <button
      ...
      {roles.includes("admin") && (
        <div style={sectionBox}>
          {sectionTitle(
            "General chat",
            "Fix display names on old messages or clear all messages.",
          )}
          ...
          >
            Fill in senderUid on old messages so chat shows current names. Run
            before renaming users for best results.
          </p>
          <button
            type="button"
          ...
          >
            Already renamed someone? Enter the <strong>old full name</strong>{" "}
            and assign those messages to the correct user:
          </p>
          <p
            style={{
              marginBottom: 8,
          ...
          >
            Chat shows first + last name only; the database stores the full name
            (including middle name). Use the full name here.
          </p>
          <div
            style={{
              display: "flex",
      ...
      {roles.includes("admin") && (
        <div style={sectionBox}>
          {sectionTitle(
            "Private chat",
            "Clear all private conversations. Cannot be undone.",
          )}
          <button
            type="button"
      ...
      {/* Classes & teachers overview – who can be assigned is set by professor/teacher/admin role in Users */}
      {(roles.includes("admin") ||
        roles.includes("professor") ||
        roles.includes("teacher")) && (
        <div style={sectionBox}>
          {sectionTitle(
            "Classes & teachers",
            "Overview of subjects and assigned teachers. Assign teachers by opening the classroom and using Edit Class Info.",
          )}
          {subHeading("Subjects and assigned teachers")}
          {classesTeachersLoading ? (
            ...
            <div style={{ overflowX: "auto" }}>
              ...
              >
                ...
                <tbody>
                  {classesTeachersOverview.map((row) => (
                    <tr
                      key={row.classId}
                    ...
                    >
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <button
                          type="button"
                          onClick={() => navigate(`/classrooms/${row.classId}`)}
                          style={{
                            ...btnPrimary,
                            padding: "6px 12px",
                            fontSize: "0.85rem",
          ...
          >
            Only users with role <strong>professor</strong>,{" "}
            <strong>teacher</strong>, <strong>admin</strong> or{" "}
            <strong>headmaster</strong> appear in the teacher list when editing
            a class. Assign roles in the Users section above.
          </p>
        </div>
      )}
      ...
      <div style={sectionBox}>
        {sectionTitle(
          "Age verification",
          "Approve or reject 18+ forum access requests.",
        )}
