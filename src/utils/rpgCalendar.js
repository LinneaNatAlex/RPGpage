...
export function getRPGCalendar(now = new Date()) {
  ...
  const weeksSinceStart = Math.floor(
    (monday - startMonday) / (7 * 24 * 60 * 60 * 1000)
  );
...
export function isExamPeriod(now = new Date()) {
  const { rpgMonth } = getRPGCalendar(now);
  return rpgMonth === 7 || rpgMonth === 8; // Juli og august
}
